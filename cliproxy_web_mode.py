...
class WebState:
    ...
    def _runtime(self, need=False):
        ...
        return {
            ...
            "auto_intv": _i(c.get("auto_check_interval_minutes"), 60, 1, 1440),
            "auto_401": "mark" if str(c.get("auto_401_action") or "").strip().lower() in ("mark", "only_mark", "仅标记") else "delete",
            "auto_quota": "delete" if str(c.get("auto_quota_action") or "").strip().lower() in ("delete", "del", "remove", "删除") else ("mark" if str(c.get("auto_quota_action") or "").strip().lower() in ("mark", "only_mark", "仅标记") else "close"),
            "auto_fill": _b(c.get("auto_allow_closed_scan", True)),
...
<title>CliproxyAccountCleaner v1.3.0</title>
<style>
